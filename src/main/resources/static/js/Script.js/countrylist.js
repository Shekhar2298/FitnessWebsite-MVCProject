...
function print_country(country_id) {
    ...
    for (var i = 0; i < country_arr.length; i++) {
        option_str.options[option_str.options.length] = new Option(country_arr[i], country_arr[i]);
    }
}
...
function print_state(state_id, state_index) {
    ...
    for (var j = 0; j < state_arr.length; j++) {
        option_str.options[option_str.options.length] = new Option(state_arr[j], state_arr[j]);
    }
}
...
/*function print_cities(city_id, city_index) {
    ...
    for (var k = 0; k < city_arr.length; k++) {
        option_str.options[option_str.options.length] = new Option(city_arr[k], city_arr[k]);
    }
}
